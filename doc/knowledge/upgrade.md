...
### 调试面板
默认了三个指标：LCP、CLS、INP
| 指标    | 缩写全称                                       | 扩写描述与用户体验意义                                                                                                                                   | 理想数值阈值 | 核心优化方向                                                              |
| :------ | :--------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------- | :------------------------------------------------------------------------ |
| **LCP** | **Largest Contentful Paint** (最大内容绘制)    | **加载性能指标**。衡量页面视口内最大的文本块或图像元素渲染完成的时间。它代表了用户感知中“页面主要内容已就绪”的速度。                                     | **≤ 2.5s**   | 优化服务器响应 (TTFB)、移除渲染阻塞的 JS/CSS、压缩大型图片素材。          |
| **CLS** | **Cumulative Layout Shift** (累计布局偏移)     | **视觉稳定性指标**。衡量页面在生命周期内发生的非预期布局移动。数值越高表示页面元素越爱“乱跳”（如广告加载撑开文字），导致用户误点。                       | **≤ 0.1**    | 为图片/视频预留尺寸空间、避免在现有内容上方插入动态内容、使用字体预加载。 |
| **INP** | **Interaction to Next Paint** (交互到下次绘制) | **交互响应性指标**。衡量用户在页面上进行点击、轻按或按键操作后，到浏览器渲染下一帧（即视觉反馈）的总延迟。它取代了旧的 FID，更全面地反映了页面的流畅度。 | **≤ 200ms**  | 减少主线程阻塞时长、分解长任务 (Long Tasks)、优化事件处理器逻辑。         |

### Performance API
暴露在 window 作用域下，[Performance](https://developer.mozilla.org/zh-CN/docs/Web/API/Performance) 接口提供了关于性能的详细信息。比如方法的执行时间、阻塞问题等。
// TODO 细说
